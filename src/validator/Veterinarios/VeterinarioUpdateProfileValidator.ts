...
export async function VeterinarioUpdateProfileValidator(req: Request) {
	const updateData: IActualizarPerfilRequest = {
		nombre: '',
		email: '',
		web: undefined,
	}
		...
		Object.keys(req.body).forEach(key => {
			if (key in updateData) {
				if (req.body[key]?.trim()) {
					updateData[key as keyof IActualizarPerfilRequest] = req.body[key]
				}
			}
		})
