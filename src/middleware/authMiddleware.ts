...
export default async function AuthenticateCheck(
	req: Request,
	res: Response,
	next: NextFunction
) {
	...
	try {
		if (authToken && authToken.startsWith('Bearer')) {
			...
			try {
				...
				const { id } = decoded as unknown as {
					id: string
					email: string
				...
				const veterinario = await Veterinario.findById(id).select(
					'-password -token -__v -confirmado'
				)
				if (!veterinario) {
					console.log(
						'Token is valid, but there is not any user associated with it'
					)
